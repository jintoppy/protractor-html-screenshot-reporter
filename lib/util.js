...
function renameAndMoveTempScreenShot(oldPath, newPath) {
...
function addHTMLReport(jsonData, baseName, options){
	var basePath = path.dirname(baseName),
		htmlFile = path.join(basePath, options.docName || 'reporter.html'),
		stream;
...
function addMetaData(metaData, baseName, descriptions, options){
	...
	var json,
		...
		file = path.join(basePath, options.combinedJsonFileName || 'combined.json');
	try {
...
module.exports = {
    ...
    , renameAndMoveTempScreenShot: renameAndMoveTempScreenShot
