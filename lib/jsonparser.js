...
stylesToAddInHtml+= "td.img-col ul li{font-weight:bold;padding-left:5px;margin-left:15px;list-style:initial;}";
...
var staticHTMLContentprefix = "<html><head><title>{{reportTitle}}</title>"+stylesToAddInHtml+scriptToAddInHtml+" </head><body style='font-family:Arial;'>";
...
function generateHTML(data){
    ...
    if(!(reporterOptions.takeScreenShotsOnlyForFailedSpecs && data.passed)) {
        var screenShotCounter = 0,
            ...
            str +=  '<td class="img-col"><ul>';

        data.manualScreenShotFiles.forEach(function(manualScreenShotFile) {
            description = data.map[manualScreenShotFile];

            str += '<li><a href="#" onclick="openModal(\'' + path.basename(manualScreenShotFile)+ '\')">' + ++screenShotCounter + ': ' + description + '</a></li>'
        });

        str +=  '<li><a href="#" onclick="openModal(\'' + path.basename(data.screenShotFile)+ '\')">Result</a></li>';
        str +=  '</ul></td>';
    }
    else{
...
function formatData(element, descArr) {
    if (currentFormattedDataIndex === descArr.length - 1) {
        var lastValueWithContent = {
            ...
            map: element.manualScreenShotNameMap,
            ...
            manualScreenShotFiles: element.manualScreenShotFiles
        };
