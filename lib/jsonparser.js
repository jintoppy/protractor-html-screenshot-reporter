...
function generateHTML(data){
    ...
    if(!(reporterOptions.takeScreenShotsOnlyForFailedSpecs && data.passed)) {
        ...
        str +=  '<td class="img-col">';    
        data.manualScreenShotFiles.forEach(function(manualScreenShotFile) {
            str += '<a href="#" onclick="openModal(\'' + path.basename(manualScreenShotFile)+ '\')">' + ++screenShotCounter + '</a><br />'
        });
        str +=  '<a href="#" onclick="openModal(\'' + path.basename(data.screenShotFile)+ '\')">Result</a>';    
        str +=  '</td>';    
    }
    else{
